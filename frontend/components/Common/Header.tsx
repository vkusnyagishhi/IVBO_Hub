...
export function Header() {
    ...
    return <>
        ...
        <HStack w='100%' borderRadius='25px 25px 0px 0px' h='80px' py='10px' bg='gray.700' pos='fixed' bottom={0} justify='space-evenly' zIndex={10} spacing={0}>
            {headerLinks.map((link, i) => <HStack key={i} bg={pathname === link.pathname ? 'rgba(0,0,150,0.15)' : 'none'} color={pathname === link.pathname ? 'blue.400' : 'white'} p='10px 16px' borderRadius='full' spacing='10px' onClick={() => router.push(link.pathname)} transition='0.2s' _hover={isLaptop ? { cursor: 'pointer', color: 'blue.200' } : {}} _active={{ color: 'gray.500' }}>
