...
export function TgLoginButton() {
    ...
    return <TelegramLoginButton
        botName={process.env.NODE_ENV === 'development' ? 'cherry4xo_validation_test_bot' : 'ivbo1123bot'}
        cornerRadius={20}
        usePic={true}
        // dataAuthUrl='http://127.0.0.1/test'
        dataOnauth={(data: TelegramUser) => {
            if (data.id === 0) return router.push('/');
            localStorage.setItem('tg_userpic', data.photo_url);

            api.post('/auth/login/telegram/generate-token', data)
                .then((res: any) => {
                    api.post('/auth/login/access-token', `username=${encodeURIComponent(res.data.username)}&password=${encodeURIComponent(res.data.short_token)}`, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
                        .then(res => {
                            localStorage.setItem('ivbo_auth', `${res.data.token_type} ${res.data.access_token}`);
                            router.refresh();
                        })
