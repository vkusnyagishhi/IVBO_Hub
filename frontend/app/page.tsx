...
import { Image, Text, ListItem, OrderedList, Accordion, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon, Spinner, VStack, HStack, Icon } from "@chakra-ui/react";
    ...
    return hw.length > 0
        ? <VStack spacing='20px' w={isLaptop ? '40%' : '90%'}>
            <HStack p='10px 20px' borderRadius='25px' color='white' bg='blue.500' justify='center' transition='0.2s' _hover={isLaptop ? { cursor: 'pointer', color: 'blue.200' } : {}} _active={{ color: 'gray.500' }} onClick={() => {
                ...
                {loading ? <Spinner /> : <Icon as={BiEdit} boxSize='30px' />}
                <Text>Изменить</Text>
            </HStack>
